...
```{r}
# Categorical variable mapped onto gear:
qplot(wt, mpg, data = mtcars, size = factor())
```

```{r}
# Continuous variable mapped onto col:
# hp
qplot(wt, mpg, data = mtcars,color =)
```
...
```{r}
#所以我们用jitter 来错位一下
qplot(factor(cyl), factor(vs), data = mtcars, geom = "")
```
...
```{r}
# 用不同大小的散点来区分不同排量的车
ggplot(mtcars, aes(x = wt, y = mpg,  = disp)) +
  geom_point()
```
